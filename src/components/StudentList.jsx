...
const StudentList = () => {
  ...
  const [students, setStudents] = useState([]);
  const [loading, setLoading] = useState(true);
  const [sendingMailIds, setSendingMailIds] = useState([]); // Track currently sending
  const [sentMailIds, setSentMailIds] = useState([]); // Track already sent
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        ...
        console.error("Failed to fetch students", err);
        toast.error("Failed to fetch students", toastStyle());
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSendMail = async (matchResultId, email) => {
    if (sendingMailIds.includes(matchResultId)) return;

    setSendingMailIds((prev) => [...prev, matchResultId]);
    ...
    try {
      ...
      if (message.toLowerCase().includes("already sent")) {
        toast.info(`Mail already sent to ${email}`, toastStyle());
        setSentMailIds((prev) => [...prev, matchResultId]);
      } else {
        toast.success(`Mail sent to ${email}`, toastStyle());
        setSentMailIds((prev) => [...prev, matchResultId]);
      }
      ...
      setSendingMailIds((prev) => prev.filter((id) => id !== matchResultId));
    }
  };
  ...
  const handleSendMailToAll = async () => {
    ...
    results.forEach((result, index) => {
      ...
      if (result.status === "fulfilled") {
        ...
        setSentMailIds((prev) => [...prev, student.id]);
      } else {
        const errorMessage =
          result.reason?.response?.data?.error ||
          result.reason?.response?.data?.detail ||
    ...
    <div className="min-h-screen p-6 bg-[#222831] text-[#DFD0B8]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {students.map((student) => {
          const isSending = sendingMailIds.includes(student.id);
          const isSent = sentMailIds.includes(student.id);
                  ...
                  <button
                    ...
                    className={`px-3 py-1 rounded transition ${
                      isSent
                        ...
                        : "bg-[#1e222a] text-[#DFD0B8] hover:bg-black"
                    }`}
                  >
                    {isSending
                      ? "Sending..."
