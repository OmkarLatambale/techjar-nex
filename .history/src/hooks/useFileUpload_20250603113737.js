...
const useFileUpload = () => {
  const uploadFile = async ({ jobId, selectedFiles }) => {
    const formData = new FormData();

    formData.append("jd_id", jobId); // ✅ Backend expects "jd_id", not "jobId"

    selectedFiles.forEach(file => {
      formData.append("resumes", file); // ✅ Key matches backend: 'resumes'
    });
